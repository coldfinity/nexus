...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    /// [Nexus patch] Multiplier applied to the computed cell height so callers
    /// can add or tighten line spacing. 1.0 = the font's natural line height.
    public var lineHeightMultiplier: CGFloat = 1.0 {
        didSet {
            guard lineHeightMultiplier != oldValue else { return }
            resetFont()
        }
    }
