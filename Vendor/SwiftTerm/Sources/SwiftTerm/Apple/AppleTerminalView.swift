...
extension TerminalView {
    ...
    func computeFontDimensions () -> CellDimension
    {
        ...
        // [Nexus patch] Scale the natural line height by lineHeightMultiplier.
    ...
    func drawTerminalContents (dirtyRect: TTRect, context: CGContext, bufferOffset: Int)
    {
        ...
        let lineDescent = CTFontGetDescent(fontSet.normal)
        let lineLeading = CTFontGetLeading(fontSet.normal)
        // [Nexus patch] Center glyphs within cells taller than the natural line
        // height (from lineHeightMultiplier) instead of pinning them to the bottom.
        let naturalHeight = lineAscent + lineDescent + lineLeading
        let extraLeading = max(cellDimension.height - naturalHeight, 0) / 2
        let yOffset = ceil(lineDescent+lineLeading+extraLeading)
        let displayBuffer = terminal.displayBuffer
